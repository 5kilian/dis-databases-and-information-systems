...
public class MovieService extends MovieServiceBase {
	...
	public MovieService() {
		...
		// getViewableMovies().ensureIndex(new BasicDBObject("tweets.coordinates", 1));
	...
	public DBCursor getByGenre(String genreList, int limit) {
		return movies.find(new BasicDBObject("genre", new BasicDBObject("$all", genreList.split(",")))).limit(limit);
	}
	...
	public DBCursor searchByPrefix(String titlePrefix, int limit) {
		return movies.find(new BasicDBObject("title", new BasicDBObject("$regex", "^" + titlePrefix))).limit(limit);
	}
	...
	public DBCursor getGeotaggedTweets(int limit) {
		return tweets.find(new BasicDBObject("coordinates", new BasicDBObject("$exists", true))).limit(limit);
	}
...
//		return found;
	...
	public void createSampleImage() {
		// Create file
		GridFSInputFile file = fs.createFile(MovieService.class.getResourceAsStream("/de/estate/resources/data/sample.png"));
	...
	private void createMovieData() {
		...
		loadJSON("/de/estate/resources/data/movies.json", movies);
		loadJSON("/de/estate/resources/data/tweets.json", tweets);
	}
